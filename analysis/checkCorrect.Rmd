...
df %>%
  ...
  select( patient_id, admission_date, separation_date, patient_days_total ) %>%
  kable()
...
df %>%
  ...
  select( patient_id, separation_date ) %>%
  kable()
...
##  missing mandatory values - patient_id, given_name
```{r}
